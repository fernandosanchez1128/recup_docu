from descarga2 import Descarga
...
#~ 
        ...
        iteraciones = cantidad_recuperados/25
        flag = True
            ...
            for child in root:
                for doi in child.findall('{http://prismstandard.org/namespaces/basic/2.0/}doi'):
                    ...
                    if (titulo != ''):
                        ...
                        self.lista_docs.append(titulo + ".txt")
...
def main():
    ...
    query = 'TITLE-ABS-KEY (cooking)  AND  DOCTYPE (ar OR re ) AND  LANGUAGE ( english )  AND  PUBYEAR  =  2016'
    ac.descargar_papers(query, 20000, 'cooking', 1)
    #consulta = raw_input('Buscar: ')
    #print consulta
    #ac.descargar_papers(consulta)
    #ac.imprimir_metadatos('computer science')
    #print ac.obtener_eid(100, 'hola')
    ac.escribir_docs('cooking',1)
    #~ 
